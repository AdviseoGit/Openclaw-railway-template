...
async function run() {
    console.log("Hämtar färsk kvälls-data från kalendern...\n");
    ...
    const countNew = (bookings) => {
        ...
        if(Array.isArray(bookings)) {
            bookings.forEach(b => {
                if(!b.isCanceled && b.created && b.created.startsWith('2026-04-20')) n++;
            });
        }
        return n;
    };
    ...
    console.log(`V.17 Onsdag (22/4): ${wedP}/300 (${Math.round((wedP/300)*100)}%) - Nya bokningar idag: ${countNew(wed)} st`);
    console.log(`V.17 Torsdag (23/4): ${thuP}/300 (${Math.round((thuP/300)*100)}%) - Nya bokningar idag: ${countNew(thu)} st`);
    console.log(`V.17 Fredag (24/4): ${friP}/510 (${Math.round((friP/510)*100)}%) - Nya bokningar idag: ${countNew(fri)} st`);
}
run();
