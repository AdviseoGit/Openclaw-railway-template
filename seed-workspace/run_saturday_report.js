...
async function run() {
    ...
    const tomorrowStr = tomorrow.toISOString().split('T')[0]; // 2026-04-26 (Sunday)
    
    console.log(`Hämtar morgon-data för Lördag (Idag: ${today})...\n`);
    ...
    console.log("\nFramåtblick - Onsdag (29/4) nästa vecka:");
    const nextWedStr = '2026-04-29';
    const nextWedBookings = await fetchFunbutler(nextWedStr);
    let nextWedP = 0;
    if(Array.isArray(nextWedBookings)) nextWedBookings.forEach(b => { if(!b.isCanceled) nextWedP += b.persons; });
    console.log(`Onsdag (29/4): ${nextWedP}/300 (${Math.round((nextWedP/300)*100)}%)`);
}
run();
