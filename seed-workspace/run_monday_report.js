...
async function run() {
    const today = new Date(); // Today is Monday 27th
    ...
    console.log(`Hämtar måndags-data från kalendern (Idag: ${today.toISOString().split('T')[0]})...\n`);
    ...
    const countNewToday = (bookings, targetDate) => {
        let n = 0;
        if(Array.isArray(bookings)) {
            bookings.forEach(b => {
                if(!b.isCanceled && b.created && b.created.startsWith(targetDate)) n++;
            });
        }
        return n;
    };
    ...
    if(ArrayArray(friBookings)) friBookings.forEach(b => { if(!b.isCanceled) friP += b.persons; });
    if(Array.isArray(satBookings)) satBookings.forEach(b => { if(!b.isCanceled) satP += b.persons; });
    
    console.log(`Onsdag (${wedStr}): ${wedP}/300 (${Math.round((wedP/300)*100)}%) - Nya bokningar idag: ${countNewToday(wedBookings, wedStr)} st`);
    console.log(`Torsdag (${thuStr}): ${thuP}/300 (${Math.round((thuP/300)*100)}%) - Nya bokningar idag: ${countNewToday(thuBookings, thuStr)} st`);
    console.log(`Fredag (${friStr}): ${friP}/510 (${Math.round((friP/510)*100)}%) - Nya bokningar idag: ${countNewToday(friBookings, friStr)} st`);
    console.log(`Lördag (${satStr}): ${satP}/510 (${Math.round((satP/510)*100)}%) - Nya bokningar idag: ${countNewToday(satBookings, satStr)} st`);
}
run();
