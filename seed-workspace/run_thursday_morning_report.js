...
async function run() {
    const today = new Date().toISOString().split('T')[0]; // 2026-04-30 (Thursday)
    const tomorrow = new Date(); tomorrow.setDate(tomorrow.getDate() + 1);
    const tomorrowStr = tomorrow.toISOString().split('T')[0]; // 2026-05-01 (Friday)
    
    console.log(`Hämtar morgon-data för Torsdag (Idag: ${today})...\n`);
    ...
    console.log(`IDAG Torsdag (30/4): ${todayP}/300 (${Math.round((todayP/300)*100)}%) - Nya bokningar idag: ${todayNewBookings} st`);
    console.log(`Imorgon Fredag (1/5): ${tomorrowP}/510 (${Math.round((tomorrowP/510)*100)}%) - Nya bokningar idag: ${tomorrowNewBookings} st`);
}
run();
